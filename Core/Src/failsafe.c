...
#define WDT_TIMEOUT_MS      4000   /* IWDG reset after 4s no kick — covers flash erase */
...
static volatile uint8_t  s_hid_ever_seen = 0; /* don't trip before first HID */
...
static void init_watchdog(void)
{
    /* IWDG clock = LSI ~32 kHz
       Prescaler = 64 → 32000/64 = 500 Hz tick
       Reload    = 1999 → 1999/500 = ~4 second timeout

       4 seconds is chosen to safely cover a flash sector erase (sector 7,
       128KB, max ~2s per datasheet) during Config_Save(). The TIM2 ISR
       cannot run while flash is being erased, so the WDT must not fire
       during that window. Config_Save() kicks the WDT manually before
       starting the erase, giving a full 4s window.

       4s is still short enough to catch a genuine firmware freeze.         */
    hiwdg.Instance       = IWDG;
...
{
    /* Pet the watchdog UNCONDITIONALLY — must happen every call regardless
       of triggered state, otherwise a triggered failsafe starves the WDT
       and causes an IWDG reset (MCU resets → device disappears from
       Device Manager and joy.cpl → blink loop).                           */
    Failsafe_KickWDT();

    /* Already triggered — motor is already stopped, don't re-evaluate     */
