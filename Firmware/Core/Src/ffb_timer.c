...
void FFB_Timer_Init(void)
{
    ...
    HAL_NVIC_SetPriority(TIM2_IRQn, 1, 0); /* priority 1 = high but below USB */
...
{
    /* Check failsafe first — if triggered, stop motor and return          */
    if (Failsafe_IsTriggered()) {
        Motor_Stop();
        return;
    }

    /* Read current wheel angle */
    float angle = ADC_GetWheelAngle();

    /* Velocity: degrees per second using 1ms fixed dt                     */
    /* Fixed dt = 0.001s because this fires at exactly 1 kHz              */
    s_velocity = (angle - s_prev_angle) * 1000.0f;

    /* Clamp velocity to prevent spikes from pot noise                     */
    if (s_velocity >  3600.0f) s_velocity =  3600.0f;  /* 10 rev/s max   */
    if (s_velocity < -3600.0f) s_velocity = -3600.0f;

    s_prev_angle = angle;

    /* Calculate FFB torque */
